...
for i in range(6,12):
    ...
    for j in range(0,6):
        ...
        this_particular_name = this_group_of_names[j]
        ...
        # plot data
        img_save = f'../Figures/FigSK/{filename_for_export}.png' 
            
        # plot and save results
        timepoints = np.array(groups_of_expt_data[this_index][0]['Time (sec)'])
        plot_and_save_mcmc_simulation_single_mg_initial_condition_FWE(M, 
                                                                     img_save,
                                                                     pid, 
                                                                     sampler, 
                                                                     groups_of_initial_conditions[this_index][j], 
                                                                     groups_of_expt_data[this_index][j],
                                                                     discard=nsteps-1, 
