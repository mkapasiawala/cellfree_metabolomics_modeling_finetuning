...
sampler, pid = py_inference(Model = M,
                            ...
                            initial_conditions = initial_conditions, # groups_of_initial_conditions[this_index],
                            nwalkers = nwalkers, 
                            nsteps = nsteps, 
                            init_seed = init_seed,
...
for i in range(6,12):
    ...
    if i==6:
        with open(f'../../Inference_Results/Sampler_and_PID_Objects/Expt_Batch_4_all_mg_concs6/{extract}_sampler.pickle', 'wb') as f:
            pickle.dump(sampler, f)
        with open(f'../../Inference_Results/Sampler_and_PID_Objects/Expt_Batch_4_all_mg_concs6/{extract}_pid.pickle', 'wb') as f:
            pickle.dump(pid, f)
        
    # plot and save results
    timepoints = np.array(groups_of_expt_data[this_index][0]['Time (sec)'])
    mase_values = plot_and_save_mcmc_simulation_multiple_mg_initial_conditions(M, 
    ...
    filename_for_mase_dict_export = f'../../Inference_Results/MASE_Values/mase_dict_expt_batch_4_all_mg_concs6_{this_index}.pickle'
