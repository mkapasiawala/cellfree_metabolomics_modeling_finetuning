...
temp_df = master_df.drop(index=master_df[(((master_df['DNA/mRNA batch'] != 6) & 
                                           (master_df['Plasmid construct'] != 'pOR1OR2-MGapt-deGFP')) & 
                                          (master_df['Plasmid construct'] != 'pTet-MGapt-deGFP') & 
                                          (master_df['Plasmid construct'] != 'pT7-MGapt-deGFP w/ 10uM T7RNAP')
                                         ) |
                                         (master_df['Extract'] != 'DAJ_MK_MY2') |
                                         (master_df['Channel'] != 'deGFP') |
                                         (master_df['DNA (nM)'] != 5) |
...
for this_index, this_name in enumerate(ordered_groups_of_interest):
    ...
    filename_for_export = f'{extract}_{fuel}_{dna_conc}nM_DNA_{fuel_conc}mM_fuel_{mg_conc}mM_Mg_{promoter}_plasmid'
    filename_for_sampler_pid_export = f'../Inference_Results/Sampler_and_PID_Objects/Expt_Batch_2/' + filename_for_export
    filename_for_image_export = f'../Inference_Results/Plots/Expt_Batch_2/' + filename_for_export + '.png'
...
filename_for_mase_dict_export = f'../Inference_Results/MASE_Values/mase_dict_expt_batch_2.pickle'
